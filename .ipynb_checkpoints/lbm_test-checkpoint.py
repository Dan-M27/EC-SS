...
# lbm Functions
#---------------------------------------------------------------------------------------------------------------------------
def lbm_sim(bdry, obstacle, maxIter, nx, ny, Re, uLB=0.04, map_color='jet'):
    ...
    #--------------------------------------------------------------
    # Run main loop
    Cl,Cd = lbm_loop(maxIter,bdry,obstacle,fin,vel,uLB,omega,nx,ny,v,t,map_color)
    return Cl, Cd

#---------------------------------------------------------------------------------------------------------------------------
# Velocity inside obstacle is non 0
def lbm_loop(maxIter, bdry, obstacle, fin, vel, uLB, omega, nx, ny, v, t, map_color):
    #==============================================================================
    #   Time-Stepping
    #==============================================================================
    t0 = ti()
    ...
    Cl = zeros(50).to(device)
    Cd = zeros(50).to(device)
    # Factor that divides hydrodynamic forces
    Cdiv = (125*uLB**2) / 2 # = 0.8333333
    Cind = 0
    ...
    for time in tqdm(range(maxIter)):
        ...
        #     plt.clim(0,0.16)
        #     plt.colorbar()
        #     plt.savefig("vel{0:03d}.png".format(time//500))
        #     # pressure field
        #     plt.clf()
        #     plt.imshow(p_cpu.T, cmap = map_color)
        #     plt.clim(0.30,0.34)
        #     plt.colorbar()
        #     plt.savefig("pr{0:03d}.png".format(time//500))
        #     # Vorticity field
        #     plt.clf()
        #     plt.imshow(vort_cpu.T, cmap = 'bwr')
        #     plt.clim(-0.16,0.16)
        #     plt.colorbar()
        #     plt.savefig("vort{0:03d}.png".format(time//500))
    tf = ti() - t0
